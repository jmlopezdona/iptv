...
// Imports
const xml2js = require('xml2js'); // https://github.com/Leonidas-from-XIV/node-xml2js
...
const utils = {
    ...
    convierteXMLaJSON: function (ficheroXML, datosXML) {
        // Devuelvo una promise dado que leo en asíncrono
        return new Promise((resolve, reject) => {
            // Prepare the parser
            let parser = new xml2js.Parser();
            console.log(`2 - Parsing de XML(movistar) a JSON(movistar)`);
            // Conversión de XML a JSON. 
            parser.parseString(datosXML, function (err, result) {
                if (err) {
                    console.log(`2 - Parsing de XML(movistar) a JSON(movistar) !! ERROR !!`);
                    reject(err);
                } else {
                    console.log(`2 - Parsing de XML(movistar) a JSON(movistar) - OK`);
                    resolve(result);
                }
            });
        });
        ...
        if (datosJSON.export.pase === undefined) {
            ...
            datosJSON.export.pase.forEach(function (pase) {
                ...
                if (index === -1) {
                    ...
                    console.log('PORQUE NO LO TENGO DADO DE ALTA EN cadenas');
                    ...
                    if ( progPreferences.cadenas[index].tvh_categoria ) {
                        categoria = progPreferences.cadenas[index].tvh_categoria;
                    }
